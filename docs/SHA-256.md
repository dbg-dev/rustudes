...
![Merkle-Damgard construction](/docs/assets/Merkle-Damgard%20hash%20construction.png)
...
- If $\left|M_1\right|=\left|M_2\right|$, then $\left|\text{Pad}\left(M_1\right)\right|=\left|\text{Pad}\left(M_2\right)\right|$.
- If $\left|M_1\right| \neq\left|M_2\right|$, then the last block of $\text{Pad}\left(M_1\right)$ is different from the last block of $\text{Pad}\left(M_2\right)$.
...
- $\text{Rot} R(A, n)$ denotes the circular right shift of $n$ bits of the binary word $A$.
...
\Sigma_{0}(X) & =\text{RotR}(X, 2) \oplus \text{RotR}(X, 13) \oplus \text{RotR}(X, 22), \\
\Sigma_{1}(X) & =\text{Rot} R(X, 6) \oplus \text{Rot} R(X, 11) \oplus \text{Rot} R(X, 25), \\
\sigma_{0}(X) & =\text{RotR}(X, 7) \oplus \text{Rot} R(X, 18) \oplus \text{ShR}(X, 3), \\
\sigma_{1}(X) & =\text{RotR}(X, 17) \oplus \text{Rot} R(X, 19) \oplus \text{ShR}(X, 10),
\end{aligned}
$$
...
>[!note] 
> Note that the indexing is from one here.

To start, the eight hash variables are set to their initial values IV that we defined in [constants](#constants).
...
$$
\begin{aligned}
T_{1} & =h+\Sigma_{1}(e)+\text{Ch}(e, f, g)+K_{i}+W_{i} \\
T_{2} & =\Sigma_{0}(a)+\text{Ma}(a, b, c) \\
h & =g \\
g & =f \\
f & =e \\
e & =d+T_{1} \\
...
![Hash function](/docs/assets//SHA-256.png)
